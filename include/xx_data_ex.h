...
struct XXXXXXXXXXX : ParentType {
    ...
    */
    ...
    template<typename T>
    struct DataFuncs<T, std::enable_if_t<IsShared_v<T> && std::is_base_of_v<SerdeBase, typename T::ElementType>>> {
    ...
    template<typename T>
    struct DataFuncs<T, std::enable_if_t<IsWeak_v<T> && std::is_base_of_v<SerdeBase, typename T::ElementType>>> {
        using U = typename T::ElementType;
        //static_assert(TypeId_v<U> > 0);
